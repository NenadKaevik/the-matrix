...
let gContext, gWidth, gHeight, // Our Canvas
...
gFontHeight = 16;
gFont = gFontHeight + 'px "Open Sans" sans-serif';
...
gFillColor = '#289628'; // #289628 - if green's your thing
gSlowFillColor = '#48B648'; // #48B648
...
rainWindowResize = function () {
  ...
  document.getElementById("q").setAttribute("width", gWidth + 'px');
  document.getElementById("q").setAttribute("height", gHeight + 'px');
  ...
  let goBoom, goBoomSlow;

  // Time to initialise all the streams
  for (let index = 0; index < gStreams; index++) {
...
renderRain = function () {
  ...
  for (let index = 0; index < gStreams; index++) {
    ...
    let text = String.fromCharCode(33 + Math.random() * 32); // Any character from 33-64. Starting at 1488 is pretty funky, too.
    // Pick an orderly line to lob it down
    let x = index * gFontHeight;
    ...
    if (y > gYTerm[index]) {
      ...
      let t = gTerminateHeight + (Math.random() * gTerminateAddHeight);
